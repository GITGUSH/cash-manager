...
public class OperacaoService
{
    ...
    //Lista as operações por filtro
    public List<Operacao> ListarContaPorData(int idConta, int idUsuario, DateTime dataInicio, DateTime dataFim)
    {
        var operacoes = new List<Operacao>();

        using var conn = Conexao.Abrir();
        using var cmd = new NpgsqlCommand(@"
        SELECT o.id_operacao, o.descricao, o.valor, o.tipo_es, o.data_operacao, o.id_categoria, c.nome
        FROM operacao o
        INNER JOIN conta c ON o.id_conta = c.id_conta
        WHERE o.id_conta = @idConta
        AND o.id_usuario = @idUsuario
        AND o.data_operacao BETWEEN @dataInicio AND @dataFim 
        ORDER BY o.data_operacao DESC", conn);

        cmd.Parameters.AddWithValue("@idConta",    idConta);
        cmd.Parameters.AddWithValue("@idUsuario",  idUsuario);
        cmd.Parameters.AddWithValue("@dataInicio", dataInicio);
        cmd.Parameters.AddWithValue("@dataFim",    dataFim.AddDays(1)); // inclui o dia final completo
